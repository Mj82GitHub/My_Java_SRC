...
//					obj.add(nodes[nodeId].getObject(i).getSeek());
	...
	/**
	 * Ищет узлы входящие в область заданную прямоугльником.
	 * 
	 * @param mbr заданная область
	 * @param nodeId узел дерева, с которого начинается поиск узла
	 * @param objs массив, в который помещаются найденные узлы
	 */
	 ...
	 * 
	 * @param mbr заданная область
	 * @param obj массив, в который помещаются найденные узлы
	 */
	...
	public GPSObject deleteObject(GPSObject obj) {
		...
		ArrayList<GPSObject> object = new ArrayList<>(1); // Смещение объекта в файле карты 
		ArrayList<RTNode> obj_node = new ArrayList<>(1); // Узел, в котором есть объект
		
		// проверяем есть ли этот объект в дереве
		findObjectsInArea(obj.getMbr(), object);
		
		if (object.size() > 0 && object.get(0).getSeek() == obj.getSeek()) {
					...
					if (node.getParent() >= 0) { // Не корень							
						for(int i = 0; i < nodes.length; i++) {
							...
							if (nodes[i].getObjects().length == 1) { 
								if (nodes[i].getObjects()[0].getSeek() == obj.getSeek()) {
